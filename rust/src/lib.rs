...
impl ChunkedBuffer {
    ...
    pub async fn push_chunk(&self, chunk: Vec<u8>) -> Result<(), SiaError> {
        let mut inner = self.inner.lock().map_err(|e| SiaError::Message(e.to_string()))?;
        if inner.closed {
...
impl App {
    #[uniffi::constructor]
    pub fn new(url: String, name: String, app_seed: String, description: String) -> Result<Self, SiaError> {
        let mut seed = [0u8; 32];
        hex::decode_to_slice(app_seed, &mut seed).map_err(|e| SiaError::Message(e.to_string()))?;
        Ok(Self {
            url,
            name,
            app_key: PrivateKey::from_seed(&seed),
            description,
...
pub struct Upload {
    ...
    result: JoinHandle<()>,
    rx: tokio::sync::Mutex<Option<oneshot::Receiver<Result<Vec<PinnedSlab>, SiaError>>>>,
}

#[uniffi::export(async_runtime = "tokio")]
impl Upload {
    pub async fn write(&self, buf: &[u8]) -> Result<(), SiaError> {
